...
export class AbmComponentsStatusComponent implements OnInit, OnDestroy {
  ...
  lookup() {
    ...
    this.totReq = rec.abms.reduce((s: number, r: any) => s + r.reqQty, 0);
    this.totRcv = rec.abms.reduce((s: number, r: any) => s + r.rcv, 0);
    this.totOpen = rec.abms.reduce((s: number, r: any) => s + (r.reqQty - r.rcv), 0);
